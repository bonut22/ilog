...
// 3. 🔥 ดึงรหัสนักศึกษา (student_id) จากฐานข้อมูลโดยตรงผ่าน user_id
$student_id = '';
...
if ($res_std && $row_std = mysqli_fetch_assoc($res_std)) {
    ...
    $_SESSION['student_id'] = $student_id; // อัปเดตฝังลง Session ไว้อีกทางด้วย
}

// 4. คำนวณชั่วโมงที่อนุมัติแล้ว (Approved)
$approved_hours = 0;
$max_hours = 480;
...
<div class="main-content">
    <div class="d-flex justify-content-between align-items-start mb-4">
        <div>
            ...
            <!-- แสดงชื่อ และ รหัสนักศึกษาอย่างถูกต้อง -->
            <p class="text-muted mb-0">
                ยินดีต้อนรับ: <strong><?php echo $full_name; ?></strong> | รหัสนักศึกษา: <strong><?php echo !empty($student_id) ? $student_id : 'ไม่พบรหัสนักศึกษา'; ?></strong>
            </p>
        </div>
        ...
        <div class="col-md-5 col-lg-4">
            <div class="card card-custom p-4 bg-white">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    ...
                    <div class="rounded-circle bg-warning bg-opacity-10 p-3 text-warning"><i class="fa-solid fa-user-clock fs-5"></i></div>
                </div>
                <h1 class="fw-bold text-orange mb-2"><?php echo $approved_hours; ?> <span class="fs-4 text-dark">/ <?php echo $max_hours; ?></span> <small class="fs-6 text-muted">ชม.</small></h1>
    ...
    <div class="card card-custom p-4 bg-white">
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <a href="daily_log.php" class="btn btn-sm btn-orange rounded-pill px-3">+ เพิ่มบันทึกงาน</a>
        </div>
                    ...
                    <?php if ($res_logs && mysqli_num_rows($res_logs) > 0): ?>
                        <?php while($row = mysqli_fetch_assoc($res_logs)): ?>
                            <tr>
                                ...
                                <td><?php echo $row['log_type'] == 'work' ? 'ทำงานปกติ' : 'ลางาน'; ?></td>
                                <td><?php echo mb_strimwidth($row['work_detail'], 0, 60, '...'); ?></td>
                                <td class="text-center">
                                    <?php 
                                        ...
                                        else echo '<span class="badge bg-warning text-dark">รอตรวจ</span>';
                                    ?>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="4" class="text-center text-muted py-3">ยังไม่มีประวัติการบันทึกงาน</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
